...
def nc2tif(data, Output_folder):
    ...
    data_arr = np.asarray(pre_data.variables['O3'])  # 属性变量名
    ...
    out_tif_name = Output_folder + '\\' + data.split('\\')[-1].split('.')[0] +data.split('\\')[-1].split('.')[4]+ '.tif'
    ...
    print(f'{data}用时{(time.time() - startime)}s')
...
def main():
    Input_folder = r'F:\data\xyz-O3\GEOS-CF\workspace\ozone\1nc'
    Output_folder = r'F:\data\xyz-O3\GEOS-CF\workspace\ozone\2tif'
