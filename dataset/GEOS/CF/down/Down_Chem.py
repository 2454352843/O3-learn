...
basepath_Chem = Config.basepath_chem
...
# ZPBL数据下载
# 根据日期下载数据
def GEOS_CF_chem_scrip(year, date):
    month = date[:2]
    day = date[2:]
    ...
    for i in range(48):
        ...
        # 2. 生成下载连接
        # Y2023/M06/D01/H12/GEOS-CF.v01.fcst.chm_tavg_1hr_g1440x721_v1.20230601_12z%2B20230602_0430z.nc4
        filename = 'Y{year}/M{month}/D{day}/H12/GEOS-CF.v01.fcst.chm_tavg_1hr_g1440x721_v1.{year}{date}_12z+{stardate}_{starhour}30z.nc4'.format(
            year=year, date=date, month=month, day=day, stardate=stardate, starhour=starhour)
        url = baseurl + filename

        folder = basepath_Chem + f'{year}_{month}_{day}'
    ...
    ##  检验
    print("开始检验")
    k = examine_chem(date,folder)

    if (k < 0):

        time.sleep(10)
        GEOS_CF_chem_scrip(year, date)


# 检验代码 检查当日数据格式和数据大小，判断当前数据是否够16个
def examine_chem(date,folder):
    mark = 0
    ...
    log2.info(f'{date}有{count}个数据,还差 {48 - count}个')
    if (count < 48):
        mark = -1
    if (mark < 0):
        return -1
...
def main_chem():
    ...
    for i in range(16):
        ...
        log2.info(f"开始下载{down_date[:2]}_{down_date[2:]}号chem数据")
        GEOS_CF_chem_scrip(year, down_date)
